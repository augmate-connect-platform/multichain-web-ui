...
<html>
	...
	<body>
		<div class="container">
			<h1 id="logo"><a href="/multichain-web-ui/"><img src="morpheus-light.png" alt="logo" />Multichain Web UI<?php if (strlen($name)) { ?> &ndash; <?php echo html($name)?><?php } ?></h1>
<?php
	if (strlen($chain)) {
		$name=@$config[$chain]['name'];
?>
...
?>
			<p class="lead"><br/>Choose from available chains to get started:</p>
		
			<p>
<?php
		foreach ($config as $chain => $rpc)
			if (isset($rpc['rpchost']))
				echo '<p class="lead">MultiChain: <a style="color:red;" href="./?chain='.html($chain).'">'.html($rpc['name']).'</a><br/>';
?>
			</p>
<?php
	}
